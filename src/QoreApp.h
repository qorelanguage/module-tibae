...
private:
   ...
   // for receive
   char* rcv_subject;
   QoreThreadLock rcv_lock;
   
   QoreHashNode* classlist;
   
   MPublisher *myPublisher;
   MRvSession *mySession;
   MSubscriber *mySubscriber;
   QoreEventHandler *myEventHandler;
   MClassRegistry *mcr;
   
   DLLLOCAL MTree *make_MTree(const char* class_name, const AbstractQoreNode* value, ExceptionSink *xsink);
   DLLLOCAL MData *instantiate_class(const AbstractQoreNode* v, const MBaseClassDescription *mbcd, ExceptionSink *xsink);
   DLLLOCAL MData *instantiate_sequence(const MSequenceClassDescription *msd, const AbstractQoreNode* v, ExceptionSink *xsink);
   DLLLOCAL MData *instantiate_modeledclass(const MModeledClassDescription *mcd, const AbstractQoreNode* v, ExceptionSink *xsink);
   DLLLOCAL MData *instantiate_union(const MUnionDescription *mud, const AbstractQoreNode* v, ExceptionSink *xsink);
   DLLLOCAL MData *do_type(int type_code, const AbstractQoreNode* v, ExceptionSink *xsink);
   DLLLOCAL MData *do_primitive_type(const MPrimitiveClassDescription *pcd, const AbstractQoreNode* v, ExceptionSink *xsink);
