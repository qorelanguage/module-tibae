...
const MBaseClassDescription *QoreApp::find_class(const char *cn, ExceptionSink *xsink) {
   const MBaseClassDescription *mbcd = 0;
   AbstractQoreNode *t = 0;
...
MData *QoreApp::do_type(int type_code, const AbstractQoreNode *v, ExceptionSink *xsink) {
   switch (type_code) {
      case TIBAE_BOOLEAN:
...
MTree *QoreApp::make_MTree(const char *class_name, const AbstractQoreNode *value, ExceptionSink *xsink) {
   ...
   std::auto_ptr<MData> md(instantiate_class(value, mbcd, xsink));
   if (*xsink) {
      return 0;
   }
   ...
   MTree *mt = new MTree();
   if ((mi = MInstance::downCast(md.get()))) {
      mi->serialize(*mt);
   }
   else if ((ms = MSequence::downCast(md.get()))) {
      ms->serialize(*mt);
   }
   else if ((mu = MUnion::downCast(md.get()))) {
      mu->serialize(*mt);
   } 
   else if ((mal = MAssocList::downCast(md.get()))) { // added by PV
     mal->serialize(*mt);
   }
...
AbstractQoreNode *QoreApp::map_mtree_to_node(MTree *msg, ExceptionSink *xsink) {
   AbstractQoreNode *rv;
...
MData *QoreApp::instantiate_modeledclass(const MModeledClassDescription *mcd, const AbstractQoreNode *v, ExceptionSink *xsink) {
   ...
   std::auto_ptr<MInstance> ma(new MInstance(mcr, mcd->getFullName()));
   ...
   while (hi.next()) {
	 ...
	 return 0;
      ...
      std::auto_ptr<MData> md(instantiate_class(t, mad->getAttributeClassDescription(), xsink));
...
MData *QoreApp::instantiate_union(const MUnionDescription *mud, const AbstractQoreNode *v, ExceptionSink *xsink) {
   ...
   std::auto_ptr<MUnion> mu(new MUnion(mcr, mud->getFullName()));
   if (h->size()) {
      ConstHashIterator hi(h);
      // get the first entry
      hi.next();
      ...
      std::auto_ptr<MData> md(instantiate_class(t, mmd->getMemberClassDescription(), xsink));
      if (*xsink)
	 return 0;
...
MData *QoreApp::instantiate_class(const AbstractQoreNode *v, const MBaseClassDescription *mbcd, ExceptionSink *xsink) {
...
AbstractQoreNode *QoreApp::sendWithSyncReply(const char *subj, const char *function_name, const AbstractQoreNode *value, int timeout, ExceptionSink *xsink) {
   ...
   std::auto_ptr<MPublisher> tp;
   try {
      QoreString s;
      s.sprintf("temp-publisher-%d", gettid());
      tp.reset(new MPublisher(this, s.getBuffer(), session_name, M_RV, subj, Mfalse));
      if (!tp.get())
	 throw MException("error", "could not create publisher!");
   }
   catch(MException &tib_e) {
...
QoreHashNode *QoreApp::receive(const char *subj, unsigned long timeout, ExceptionSink *xsink) {
   ...
   try {
	 ...
	 // assign "msg" member
	 h->setKeyValue("msg", myEventHandler->msgNode, 0);
	 myEventHandler->msgNode = 0;

	 printd(5, "QoreApp::receive() subject=%s\n", myEventHandler->subject.c_str());
	 // assign "subject" member
	 h->setKeyValue("subject", new QoreStringNode(myEventHandler->subject.c_str()), 0);

	 printd(5, "QoreApp::receive() replySubject=%s\n", myEventHandler->replySubject.c_str());
	 // assign "replySubject" member
	 h->setKeyValue("replySubject", new QoreStringNode(myEventHandler->replySubject.c_str()), 0);
	 rv = h;
      }
      else
...
QoreEventHandler::QoreEventHandler(QoreApp *pMApp) : MEventListener() {
   myQoreApp = pMApp;
   count = 0;
   msgNode = 0;
}
