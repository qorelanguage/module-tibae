...
void TIBAE_constructor(QoreObject *self, const QoreListNode *params, ExceptionSink *xsink) {
   ...
   if (!(p0 = test_string_param(params, 0)) || !(p1 = test_hash_param(params, 1))) {
      xsink->raiseException("TIBCO-PARAMETER-ERROR", "invalid parameters passed to TibcoAdapter::constructor(), expecting session name (string), properties (object), [classlist (object), service (string), network (string), daemon (string)]");
...
static AbstractQoreNode *TIBAE_sendSubjectWithSyncReply(QoreObject *self, QoreApp *myQoreApp, const QoreListNode *params, class ExceptionSink *xsink) {
   const QoreStringNode *p0, *p1;
...
static AbstractQoreNode *TIBAE_receive(QoreObject *self, QoreApp *myQoreApp, const QoreListNode *params, class ExceptionSink *xsink) {
   const QoreStringNode *p0;
